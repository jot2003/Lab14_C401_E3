...
class RetrievalEvaluator:
    ...
    def calculate_hit_rate(self, expected_ids: List[str], retrieved_ids: List[str], top_k: int = 3) -> float:
        ...
        If no expected IDs are specified (e.g. adversarial/OOS cases), return 1.0
        because there is no retrieval expectation to violate.
        """
        if not expected_ids:
            return 1.0
    ...
    def calculate_mrr(self, expected_ids: List[str], retrieved_ids: List[str]) -> float:
        ...
        If no expected IDs are specified, return 1.0 (trivially satisfied).
        """
        if not expected_ids:
            return 1.0
